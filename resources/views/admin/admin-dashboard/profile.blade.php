    ...
    <div class="card p-4">
        <div class=" image d-flex flex-column justify-content-center align-items-center"> <button
                class="btn btn-secondary"> <img src="https://i.imgur.com/wvxPV9S.png" height="100"
                    width="100" /></button> <span class="name mt-3">{{$record->name}}</span> <span
                class="idd">Email : {{$record->email}}</span>
            <div class="d-flex flex-row justify-content-center align-items-center gap-2"> <span
                    class="idd1">{{$record->dob}}</span> <span><i class="fa fa-copy"></i></span> </div>
...
<!--            
            ...
            <div class=" px-2 rounded mt-4 date "> <span class="join">Joined:{{$record->created_at}}</span> </div>
        </div>
    </div>
</div>
@endsection()
